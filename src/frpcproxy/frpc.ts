/// FRPC Parser, extracted and modified from the JAK library, © 2010 Seznam.cz, a.s.
/// Available under MIT license
/// See https://github.com/seznam/JAK/blob/master/util/frpc.js
 ...
 * @group jak-utils
 */
export default class FRPC {
    public NAME = "JAK.FRPC";
    public VERSION = "1.2";

    public static TYPE_MAGIC		= 25;
    public static TYPE_CALL		= 13;
    public static TYPE_RESPONSE	= 14;
    public static TYPE_FAULT		= 15;

    public static TYPE_INT		= 1;
    public static TYPE_BOOL		= 2;
    public static TYPE_DOUBLE	= 3;
    public static TYPE_STRING	= 4;
    public static TYPE_DATETIME	= 5;
    public static TYPE_BINARY	= 6;
    public static TYPE_INT8P		= 7;
    public static TYPE_INT8N		= 8;
    public static TYPE_STRUCT	= 10;
    public static TYPE_ARRAY		= 11;
    public static TYPE_NULL		= 12;

    private _hints: HintObject|string = null;
    private _path: any[] = [];
    ...
    public parse(data: number[]): any {
        ...
        if (magic1 !== 0xCA || magic2 !== 0x11) { 
            this._data = [];
            throw new Error("Missing FRPC magic, found [" + magic1 + ", " + magic2 + "]"); 
        }
     ...
     */
    public serializeCall(method: string, data: any[], hints?: HintObject | string): number[] {
     ...
     * @returns {number[]}
     */
    public serialize(data: any, hints?: HintObject|string): number[] {
        var result: number[] = [];
        this._path = [];
        this._hints = hints;
        ...
        */
        ...
        switch (type) {
            ...
            case FRPC.TYPE_STRUCT:
                ...
                while (members--) { this._parseMember(structResult); }
                return structResult;
            ...
            case FRPC.TYPE_ARRAY:
                ...
                var lengthBytes = (first & 7) + 1;
                var members = this._getInt(lengthBytes);
                while (members--) { arrayResult.push(this._parseValue()); }
                return arrayResult;
            ...
            case FRPC.TYPE_INT:
                ...
                var intResult = this._getInt(length);
                if (intResult >= max/2) { intResult -= max; }
                return intResult;
            ...
            case FRPC.TYPE_BINARY:
                var lengthBytes = (first & 7) + 1;
                var length = this._getInt(lengthBytes);
                var binaryResult: number[] = [];
                while (length--) { binaryResult.push(this._getByte()); }
                return binaryResult;
    ...
    private _append(arr1: any[], arr2: any[]) {
        var len = arr2.length;
        for (var i=0;i<len;i++) { arr1.push(arr2[i]); }
    }

    private _parseMember(result: Object) {
        var nameLength = this._getInt(1);
        var name = this._decodeUTF8(nameLength);
        (<any>result)[name] = this._parseValue();
    }

    /**
     * In little endian 
     */
    private _getInt(bytes: number) {
        var result = 0;
        var factor = 1;
    ...
    private _decodeUTF8(length: number) {
        /* pouzite optimalizace:
    ...
    private _encodeUTF8(str: string): number[] {
        var result: number[] = [];
    ...
    private _serializeValue(result: number[], value: any): void {
        if (value === null) {
        ...
        switch (typeof(value)) {
            ...
            case "number":
                if (this._getHint() === "float" || !Number.isInteger(value)) { /* float */
                    ...
                    var intData = this._encodeInt(Math.abs(value));
                    first += (intData.length-1);
            ...
            case "boolean":
                var boolData = FRPC.TYPE_BOOL << 3;
                if (value) { boolData += 1; }
                result.push(boolData);
            break;
    ...
    private _serializeArray(result: number[], data: any[]): void {
    ...
    private _serializeStruct(result: number[], data: Object): void {
        var numMembers = 0;
        ...
        for (var p in data) {
            ...
            this._append(result, strData);
            this._path.push(p);
            this._serializeValue(result, (<any>data)[p]);
            this._path.pop();
        }
    }

    private _serializeDate(result: number[], date: Date): void {
     ...
     */
    private _encodeInt(data: number): number[] {
        if (!data) { return [0]; }
     ...
     */
    private _encodeDouble(num: number): number[] {
        var result: number[] = [];
        ...
        var sign: number, exponent: number, fraction: number;
        if (isNaN(num)) {
    ...
    private _getHint(): string {
        ...
        if (typeof(this._hints) !== "object") { return <string>this._hints; } /* skalarni varianta */
        var hints = <HintObject>this._hints;
        return hints[this._path.join(".")] || null;
    }
}
